...
#./deployment.sh -c false -d true -p false -b ops-cicd-olx -s ncc-fgx-olx-1663
# provision subservices
#./deployment.sh -c false -d false -p false -b ops-cicd-olx -s ncc-ngfw-olx-711
# retry
#./deployment.sh -c true -d false -p true -b ops-cicd-olx
...
deployment() {
  ...
  if [[ "$PROVISION_PROJ" != false ]]; then
    ...
    echo "wait 45 sec for SA roles to propagate"
    sleep 45
